...
5. 仓库内部其它嵌套目录存在 `.git/core.worktree` 错位：如果该路径已登记到当前 `.gitmodules`，且 URL 与借用的旧 gitdir remote 一致，则复制一份独立 gitdir 给新路径；否则只诊断并中止。
6. 同一 `.gitmodules` section 的路径发生迁移：先安全修正 `.git/modules` 中的 `core.worktree`，再检查新路径是否为 clean 的有效子模块。
7. 旧 gitlink 已经从当前 `.gitmodules` 移除，且路径不存在或只是空目录：允许后续 `git add -A -- .` 暂存删除。
8. 子模块已存在且有内部修改：立即中止，不丢弃、不提交、不替用户选择。
