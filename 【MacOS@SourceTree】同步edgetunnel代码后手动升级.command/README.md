...
```shell
~/SourceTree.command/【MacOS@SourceTree】同步edgetunnel代码后手动升级.command/【MacOS@SourceTree】同步edgetunnel代码后手动升级.command \
  ../../../edgetunnel
```
...
| 运行的脚本 | `~/SourceTree.command/【MacOS@SourceTree】同步edgetunnel代码后手动升级.command/【MacOS@SourceTree】同步edgetunnel代码后手动升级.command` |
| 参数 | `$REPO` |
| 在单独窗口中打开 | 勾选 |
| 显示完整输出 | 勾选 |
...
xattr -dr com.apple.quarantine "~/SourceTree.command/【MacOS@SourceTree】同步edgetunnel代码后手动升级.command" 2>/dev/null || true
```
...
- 如果仍想在终端里强制使用纯文本，可以这样运行：
  ...
  ```shell
  JOBS_PLAIN_OUTPUT=1 ~/SourceTree.command/【MacOS@SourceTree】同步edgetunnel代码后手动升级.command/【MacOS@SourceTree】同步edgetunnel代码后手动升级.command \
    ../../../edgetunnel
  ```
...
```text
$TMPDIR/【MacOS@SourceTree】同步edgetunnel代码后手动升级.log
```

排查时可以开另一个终端实时看：

```shell
tail -f "$TMPDIR/【MacOS@SourceTree】同步edgetunnel代码后手动升级.log"
```
...
```text
运行的脚本 = ~/SourceTree.command/脚本.command/脚本.command
参数       = $REPO
```
