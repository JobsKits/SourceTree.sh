...
- 本自述文件对应脚本：`【MacOS@SourceTree】📦双击打包Flutter.android.command`。
- 脚本原始位置：`JobsGenesis@JobsCommand.SourceTree`。
- 脚本定位：处理 Android 模拟器、SDK、构建或运行相关流程。 用于 SourceTree 自定义操作入口。
...
```shell
chmod +x './【MacOS@SourceTree】📦双击打包Flutter.android.command'
'./【MacOS@SourceTree】📦双击打包Flutter.android.command'
```
...
- 运行日志默认写入：`$TMPDIR/【MacOS@SourceTree】📦双击打包Flutter.android.log`。
