# `【MacOS@SourceTree】🐦Flutter运行setup.command.command`
...
- 本自述文件对应脚本：`【MacOS@SourceTree】🐦Flutter运行setup.command.command`。
- 脚本原始位置：`JobsGenesis@JobsCommand.SourceTree`。
- 脚本定位：用于 SourceTree 自定义操作入口。定位 Flutter 工程里的 `setup.command`，再交给系统 Terminal 执行。
...
| 主要标签 | `SourceTree, Flutter, setup.command, Terminal` |
| 是否涉及 Homebrew | `否` |
| 是否可能联网 | `取决于 setup.command 内部逻辑` |
| 是否含高风险命令 | `本脚本不含；setup.command 内部以实际工程脚本为准` |
| zsh 静态检查 | `已通过 zsh -n` |
...
- 本脚本不会替代 `setup.command`，只负责把 Sourcetree 触发动作桥接到系统 Terminal。
